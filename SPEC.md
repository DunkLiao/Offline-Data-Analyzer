...
| 視覺化層 | 將分析結果轉為五數摘要、直方圖或常見值長條圖模型並繪製 Canvas；進階模式提供散佈圖、交叉頻率表、箱型圖、品質總覽與時間序列 | 不使用外部圖表套件 |
...
| `advChartKind` | 進階圖表種類：`scatter`、`crosstab`、`boxplot`、`overview` 或 `line` |
...
欄位視覺化流程：分析完成後篩選可視覺化欄位；數值欄位預設五數摘要並可切換直方圖，字串欄位顯示前 5 常見值長條圖。使用者可切換至「進階圖表」模式，依圖表種類動態選擇欄位（散佈圖需兩個數值欄位、交叉表需兩個字串欄位、箱型圖需一個數值欄位、時間序列需一個日期與一個數值欄位、品質總覽不需選欄位）。切換欄位、種類或類型時，只重新建立圖表模型並重繪 Canvas，不重新解析 CSV 或改寫 IndexedDB。
...
單欄圖表與進階圖表共用欄位視覺化區塊、Canvas 與 PNG 下載按鈕。所有圖表必須使用目前欄位的分析結果與原始資料計算，無有效資料時顯示提示並停用下載。Canvas 需保留固定繪圖高度與足夠左右邊界，依實際顯示寬度與裝置像素比調整解析度；交叉表與品質總覽依欄位數量自動放大繪圖尺寸並以捲動承接，避免標題、座標標籤或數值遭裁切。
