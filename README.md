...
這是一個完全在瀏覽器本機端執行的 CSV 資料品質與分布分析工具。使用者只要開啟 `index.html`，即可匯入本機 CSV，查看欄位型態、缺失值、缺失率、數值分布統計，並產生單欄位圖表與多欄位進階圖表（散佈圖、交叉頻率表、箱型圖、品質總覽、時間序列、相關係數熱力矩陣、缺失值分布熱力圖、分組彙總長條圖、平行座標圖），也能下載 PNG 圖片。資料會儲存在瀏覽器 IndexedDB，不會上傳到雲端，也不需要安裝 Python、Node、Docker 或資料庫。
...
- 欄位視覺化：可選擇單一欄位，依欄位型態查看五數摘要圖、直方圖、ECDF 累積分布圖或常見值長條圖；並可切換至「進階圖表」查看散佈圖、交叉頻率表、箱型圖、資料品質總覽、時間序列折線圖、相關係數熱力矩陣、缺失值分布熱力圖、分組彙總長條圖或平行座標圖。
...
2. 若欄位為「數值」，可切換：
   ...
   - 累積分布圖 (ECDF)：以階梯線顯示小於等於各數值的資料比例，並以紅色虛線標註 P25、中位數與 P75。
...
| 相關係數熱力矩陣 | 不需選擇 | 所有數值欄位（上限 30 欄）兩兩 Pearson 相關；紅色為正相關、藍色為負相關 |
| 缺失值分布熱力圖 | 不需選擇 | 資料列分帶（上限 48 帶）×欄位的缺失比例熱力圖，可看出缺失集中的位置 |
| 分組彙總長條圖 | 一個字串欄位＋一個數值欄位 | 依類別顯示平均值／中位數／總和長條與筆數；類別過多時彙整「其他」 |
| 平行座標圖 | 不需選擇 | 所有數值欄位（上限 12 軸）正規化後的資料列連線，疊加各軸中位數紅線 |
...
| `07_utf8_new_charts.csv` | 新圖表：多數值欄位（含高相關與全同值欄）、分類欄與集中於前段的缺失欄位 |
...
3. 確認繁中文字沒有亂碼。
4. 確認缺失值欄位以紅色提示。
5. 選擇數值欄位，確認可切換五數摘要、直方圖與累積分布圖 (ECDF)，並可下載 PNG。
...
7. 切換至「進階圖表」，逐一確認：
   ...
   - 相關係數熱力矩陣：匯入 07 後不需選欄位，確認「總價 × 單價」為深紅正相關、「評分」全欄顯示 -，可下載 PNG。
   - 缺失值分布熱力圖：確認「備註」欄前段深紅、後段轉淺，訊息列顯示總缺失格數與比例。
   - 分組彙總長條圖：選擇「分類 × 總價」，切換平均/中位數/總和，確認每條顯示 n= 筆數。
   - 平行座標圖：確認「評分」軸標示「全同值」、紅色中位數線顯示，可下載 PNG。
